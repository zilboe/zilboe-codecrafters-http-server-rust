...
// fn handle_connect(stream: TcpStream, read_buff: Vec<u8>) {
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    loop {
        ...
        tokio::spawn(async move {
            let mut byte_r = [0; 1024];
            loop {
                match socket.read(&mut byte_r).await {
                    Ok(_) => {
                ...
                let mut is_gzip: bool = false;
                let recv_string =
                    String::from_utf8((&byte_r).to_vec()).expect("utf-8 to string error");
                //println!("{}", recv_string);
                ...
                for recv_split_one in &recv_split {
                    if recv_split_one.starts_with("Accept-Encoding:") {
                        let recv_split_skip_encoding_head = "Accept-Encoding:".len();
                        let recv_split_skip_encoding =
                            &recv_split_one[recv_split_skip_encoding_head..];
                        let recv_split_one_encoding: Vec<&str> =
                            recv_split_skip_encoding.split(',').collect();
                        for recv_encoding_split in recv_split_one_encoding {
                            if recv_encoding_split.trim().starts_with("gzip") {
                                is_gzip = true;
                            }
                ...
                if recv_split[0].starts_with("GET /index.html") {
                    ...
                    for user_agent_split in recv_split {
                        if user_agent_split.starts_with("User-Agent: ") {
                            ...
                            if is_gzip {
                                write_buff
                                    .extend_from_slice("Content-Encoding: gzip\r\n".as_bytes());
                            }
                    ...
                    if path_file.exists() {
                        ...
                        match path_file_open.read_to_end(&mut file_open_buff) {
                            Ok(_) => {
                                let file_buffer_len = file_open_buff.len();
                                let file_open_buff_to_string =
                                    String::from_utf8_lossy(&file_open_buff);
                                ...
                                if is_gzip {
                                    ...
                                    write_buff.extend_from_slice(write_file_buff.as_bytes());
                                    write_buff
                                        .extend_from_slice(file_open_buff_to_string.as_bytes());
                                }
                            }
                            Err(_) => {
                                println!("read file to buffer error");
                    ...
                    for get_post_recv_head in recv_split {
                        if get_post_recv_head.starts_with("Content-Length:") {
                            let skip_post_content_len = "Content-Length: ".len();
                            let get_post_content_size =
                                &get_post_recv_head[skip_post_content_len..];
                            let get_post_content_size: usize = get_post_content_size
                                .parse()
                                .expect("POST: Get content-len Error");

                            let mut create_file: File =
                                fs::File::create(post_head_filename.clone())
                                    .expect("create file error");
                            let request_post_content =
                                &request_post_content[..get_post_content_size];
                            create_file
                                .write(request_post_content.as_bytes())
                                .expect("POST: write data to file error");
                ...
                let _ = socket.write_all(&write_buff).await;
                //let strs = String::from_utf8_lossy(&byte_r);
                //println!("{}", strs);
                // handle_connect(socket, (&byte_r[0..n]).to_vec());
            }
        });
    }
}
